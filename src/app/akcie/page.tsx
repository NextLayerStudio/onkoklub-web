...
import { Leaf, Brain, Stethoscope, Coffee, MapPin, Circle } from "lucide-react";

const CITIES = ["Bratislava", "Nitra", "Košice", "Poprad", "Ďalšie mestá pribudnú"];

const EVENT_TYPES = [
  { icon: Leaf,        accent: "#FDA4C7", title: "Joga workshopy",       desc: "Workshopy s Petrou Joga — jemné cvičenie špeciálne pre onkologických pacientov. Bez nároku na kondíciu, v bezpečnom prostredí." },
  { icon: Brain,       accent: "#6F2380", title: "Mindfulness a meditácia", desc: "Skupinové sedenia s odborníkmi. Techniky zvládania stresu, dýchanie, prítomnosť." },
  { icon: Stethoscope, accent: "#FDA4C7", title: "Odborné stretnutia",    desc: "Stretnutia s lekármi, psychológmi a odborníkmi. Priestor na otázky a zdieľanie skúseností v malej skupine." },
  { icon: Coffee,      accent: "#6F2380", title: "Komunity meetupy",      desc: "Neformálne stretnutia členov OnkoKlubu. Káva, rozhovor, ľudia s podobnou skúsenosťou." },
];

const STEPS = [
  { title: "Pozri si kalendár", desc: "Všetky nadchádzajúce akcie nájdeš v členskej sekcii, filtrované podľa tvojho kraja." },
  { title: "Rezervuj si miesto", desc: "Jedným kliknutím — potvrdenie príde e-mailom a pripomienka 24 hodín vopred." },
  { title: "Príď a spoznaj komunitu", desc: "Akcie sú pre členov zadarmo alebo so zvýhodnenou cenou. Kapacita je obmedzená." },
];
      ...
      <section className="px-5 pb-12">
        ...
        <div className="flex flex-col gap-3">
          {EVENT_TYPES.map((e) => {
            const Icon = e.icon;
            return (
              <div key={e.title} className="flex items-center gap-4 bg-white rounded-2xl p-4 border border-[#FDA4C7]/15">
                <div className="w-11 h-11 rounded-xl flex items-center justify-center shrink-0" style={{ backgroundColor: e.accent }}>
                  <Icon size={18} className="text-white" strokeWidth={2} />
                </div>
                <div>
                  <h3 className="font-black text-[#6F2380] text-[15px] leading-tight mb-0.5">{e.title}</h3>
                  <p className="text-[#6F2380]/55 text-[13px] leading-relaxed">{e.desc}</p>
                </div>
              </div>
            );
          })}
        ...
        <div className="flex flex-col gap-5">
          {STEPS.map((s, i) => (
            <div key={s.title} className="flex gap-4 items-start">
              ...
              <div className="pt-1">
                <p className="font-black text-[#6F2380] mb-1 text-[15px]">{s.title}</p>
