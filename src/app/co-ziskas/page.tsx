...
export default function CoZiskasPage() {
  return (
    <main className="min-h-screen bg-[#FFF3F9] font-sans">
      ...
      {/* Veľký screenshot platformy */}
      <section className="px-5 pb-14">
        {/* [IMG-25] Dashboard platformy */}
      ...
      {/* Features — každý ako samostatná sekcia */}
      {FEATURES.map((f, fi) => {
        const Icon = f.icon;
        const isEven = fi % 2 === 0;
        return (
          <section key={f.title} className="px-5 pb-16">
            {/* Label + nadpis */}
            ...
            <h2 className="text-[1.8rem] font-black text-[#6F2380] leading-[1.15] mb-3">
              {f.headline}
            </h2>
            <p className="text-[#6F2380]/65 text-[15px] leading-relaxed mb-6">{f.desc}</p>

            {/* Screenshot placeholdery */}
            <div className={`flex flex-col gap-3 mb-6 ${f.imgs.length === 1 ? "" : ""}`}>
              {f.imgs.length === 1 ? (
                <div
                ...
                >
                  <p
                    ...
                    style={{ color: (isEven ? "#FDA4C7" : "#6F2380") + "60" }}
                  >
                    {f.imgs[0]}
                  </p>
                </div>
                ...
                <>
                  ...
                  >
                    <p className="text-xs font-semibold px-4 text-center" style={{ color: f.accent + "70" }}>
                      {f.imgs[0]}
                    </p>
                  </div>
                    ...
                    {f.imgs.slice(1).map((img) => (
                      ...
                      >
                        <p
                          ...
                          style={{ color: (isEven ? "#6F2380" : "#FDA4C7") + "60" }}
                        >
                          {img}
                        </p>
                      </div>
            ...
            {fi < FEATURES.length - 1 && (
              <div className="h-px bg-[#FDA4C7]/15 mt-10" />
            )}
          </section>
        );
      })}
